...
cycling." But Opentrons Python, liquid handler (STAR), and Agilent Bravo
...
**backend**. The Flex is our prototype rig; the liquid handler / Bravo are where
...
```
   ...
   examples/*.py ─▶ │   ProtocolSpec (spec.py) │ ─┐  the portable asset
                    ...
                    └─────────────────────────┘  │
                                                  ...
                                                  ├─▶ pylabrobot_backend -> liquid handler .py (PyLabRobot)
                                                  ├─▶ worklist_backend   -> CSV (STAR/Bravo import)
                                                  └─▶ liquid_handler_backend   -> native Venus method (Phase 3)
```
...
# liquid handler / Agilent Bravo worklist (vendor-neutral transfer list):
...
| Native method + accuracy tuning (liquid classes for 1-3% CV) | ⏳ Phase 3 | `liquid_handler_backend` (the seam, stubbed) |
