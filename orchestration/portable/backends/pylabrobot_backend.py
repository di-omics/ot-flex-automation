"""Compile a ProtocolSpec -> a PyLabRobot script for the liquid handler.

PyLabRobot (PLR) is a hardware-agnostic Python liquid-handling framework with a
real liquid handler backend - a far better liquid handler target than a raw worklist
...
def render(spec: ProtocolSpec) -> str:
    ...
    L: list[str] = []
    w = L.append
    w('"""AUTO-GENERATED from a portable ProtocolSpec - liquid handler via PyLabRobot.')
    ...
    w(f"    {', '.join(classes)})")
    w("")
    w("USE_CHATTERBOX = True   # False -> real liquid handler")
