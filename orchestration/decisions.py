...
concentration source) can drive them automatically instead of a manual
fluorometric DNA-quantification result.
...
Rules transcribed from the protocols (keep in sync with the pause text):
  ...
  WGS, post-amplification QC checkpoint (whole_genome_seq_flex.py):
    fluorometric dsDNA yield > 800 ng average; fragment size ~1275 bp
...
# ---- WGS -----------------------------------------------------------------
WGS_MIN_YIELD_NG = 800.0     # expected fluorometric dsDNA yield
WGS_NORMALIZE_NG_UL = 2.0    # target normalized plate


def wgs_amplification_qc(total_ng: float, size_bp: float | None = None) -> Decision:
    """Post-amplification checkpoint: normalize or flag a low/failed result."""
    if total_ng < 0.5 * WGS_MIN_YIELD_NG:
        return Decision(Action.FLAG,
                        f"{total_ng:.0f} ng << {WGS_MIN_YIELD_NG:.0f} expected -> review amplification before prep",
...
if __name__ == "__main__":
    ...
    print("WGS 1100 ng          :", wgs_amplification_qc(1100, 1275))
    print("WGS 250 ng           :", wgs_amplification_qc(250))
