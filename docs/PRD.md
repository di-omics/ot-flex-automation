...
- **Portable intent:** volumes, source-to-destination maps, step order, and QC
  gates are separate from robot-specific API calls.
...
1. `protocols/` contains runnable Opentrons protocols.
2. `orchestration/portable/` contains platform-neutral protocol intent and
   renderers.
...
- Reconcile deck layouts, volumes, timings, and pauses with locally validated
  WGS, methylation-sequencing, and TIP-seq SOPs.
...
- Wet-lab precision and library-quality thresholds are documented before
  production use.
